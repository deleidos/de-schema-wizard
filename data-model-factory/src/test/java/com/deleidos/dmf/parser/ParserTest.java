...
import com.deleidos.dmf.framework.ResourceLoader;
...
public abstract class ParserTest extends DMFMockUpEnvironmentTest {
	...
	public void testNonEmptyDataSampleBean() throws IOException, SAXException, TikaException, AnalyzerException {
		...
		int i = 0;
		for(HeadlessResource ds : streamSources) {
			...
			File file = AnalyticsEmbeddedDocumentExtractor.getOrCreateExtractedContentDirectory(
					params.getUploadFileDir(), params.get(File.class).getName(), "-embedded");
			if(new File(ds.getFilePath()).exists()) {
				params.set(File.class, new File(ds.getFilePath()));
			}
			params.setExtractedContentDir(file.getAbsolutePath());
